...
struct tx80211 {
	...
	/* Record the starting MAC address to restore if changed 记录起始MAC地址，以便在更改时恢复*/
	uint8_t startingmacset;
	uint8_t startingmac[6];

	/* Extra space for use by the driver code 驱动程序代码使用的额外空间 */
	void *extra;

	/* Callthroughs to functions installed by the setup when the injector is
	 * defined. 定义注入器时，指向安装程序安装的函数的调用*/
